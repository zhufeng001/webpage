...
timeout=0
hiddenmenu
title Fronware (2.6.x86_64)
        root (hd0,%s)
        kernel /vmlinuz-2.6.32-71.14.1.el6.x86_64 ro root=UUID=%s rd_NO_LUKS rd_NO_LVM rd_NO_MD rd_NO_DM LANG=zh_CN.UTF-8 KEYBOARDTYPE=pc KEYTABLE=us crashkernel=auto rhgb quiet nmi_watchdog=0 vga=791
...
def installvserver(event):
    ...
    os.system("/bin/sync")
    time.sleep(20)
    os.system("/usr/bin/python /usr/vmd/plyscript.pyc")
    os.system("/bin/sync")
    time.sleep(20)
